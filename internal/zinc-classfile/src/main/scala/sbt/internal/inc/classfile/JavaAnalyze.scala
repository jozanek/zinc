...
private[sbt] object JavaAnalyze {
    ...
    for ((source, classFiles) <- sourceToClassFiles) {
      ...
      val localClassesToSources =
        if (classfileApiOnly) {
          ...
          (for {
            cf <- classFiles
            if classFileSourceName(cf).isEmpty // local or anonymous
   ...
   * classfiles alone (the EnclosingMethod attribute walked up via [[classFileSourceName]]) — the
